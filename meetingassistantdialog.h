#ifndef MEETINGASSISTANTDIALOG_H
#define MEETINGASSISTANTDIALOG_H

#include <QDialog>
#include <QAudioInput>
#include <QAudioSource>
#include <QMediaDevices>
#include <QFile>
#include <QIODevice>
#include <QNetworkAccessManager>
#include <QMessageBox>
#include <QNetworkReply>
#include <QJsonDocument>
#include <QJsonObject>
#include <QJsonArray>
#include <QHttpMultiPart>
#include <QHttpPart>
#include <QFileInfo>
#include <QFileDialog>

namespace Ui {
class MeetingAssistantDialog;
}

class MeetingAssistantDialog : public QDialog
{
    Q_OBJECT

public:
    explicit MeetingAssistantDialog(QWidget *parent = nullptr);
    ~MeetingAssistantDialog();


    //调用音频转文字智能体接口
    void getProfile(); //获取应用id
    void getChatId();  //获取会话id

    //调用会议纪要智能体接口
    void Meeting_getProfile(); //获取会议纪要应用id
    void Meeting_getChatId();  //获取会议纪要会话id

    void Meeting_sendSummaryRequests(const QString &finalText); //调用获取summary等


private slots:
    void on_uploadBtn_clicked();

signals:
    //收到appid的信号
    void appIdReceived(const QString &appId);

    //收到chatid的信号
    void chatIdReceived(const QString &chatId);

    //收到会议纪要appid的信号
    void m_appIdReceived(const QString &appId);

    //收到会议纪要chatid的信号
    void m_chatIdReceived(const QString &chatId);

    // 当收到模型回复时
    void messageReceived(const QString &reply);

    // 错误处理
    void errorOccurred(const QString &errorMsg);

private:
    Ui::MeetingAssistantDialog *ui;

    void setupAudioInput();
    void writeWavHeader(QFile *file, qint64 dataSize = 0);

    void onStartRecording();
    void onStopRecording();
    void onUploadAudio(const QString &filePath);
    void onClose();

    void sendTranscriptionRequest(const QString &fileId, const QString &fileUrl, const QString &fileName, const QString &filePath);

    // 音频录制相关
    QAudioSource *audioSource;
    QIODevice *audioDevice;
    QFile *outputFile;
    QString currentFileName;
    bool isRecording;

    // 音频格式
    QAudioFormat audioFormat;

    QNetworkAccessManager *manager;
    QString baseUrl;
    QString apiKey;
    QString profileUrl;
    QString currentMessage; //流式消息
    QString appId;  //应用id
    QString chatId;  //会话id
    QByteArray buffer; // 用来拼接流式分片

    QString finalText;  //记录音频转录结果

    QString m_apiKey;
    QString m_appId;  //会议纪要应用id
    QString m_chatId;  //会议纪要会话id

    QMessageBox *m_processingBox = nullptr; //保存消息框指针

};

#endif // MEETINGASSISTANTDIALOG_H
